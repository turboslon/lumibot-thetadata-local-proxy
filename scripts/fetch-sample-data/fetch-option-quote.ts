/**
 * Fetch Sample Data Script: Option History Quote
 * 
 * Calls ThetaData Terminal directly and stores raw request/response.
 * 
 * Usage: npx ts-node scripts/fetch-sample-data/fetch-option-quote.ts
 */
...
async function fetchAndCapture(): Promise<void> {
    const endpoint = '/v3/option/history/quote';
    const queryParams = {
        symbol: 'AAPL',
        expiration: '20250117',
        strike: '150',
    ...
    try {
        ...
        // Save to fixtures
        const fixtureDir = path.join(__dirname, '../../test/fixtures/option-history-quote');
