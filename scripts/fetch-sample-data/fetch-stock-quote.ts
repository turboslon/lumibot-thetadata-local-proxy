/**
 * Fetch Sample Data Script: Stock History Quote
 * 
 * Calls ThetaData Terminal directly and stores raw request/response.
 * 
 * Usage: npx ts-node scripts/fetch-sample-data/fetch-stock-quote.ts
 */
...
async function fetchAndCapture(): Promise<void> {
    const endpoint = '/v3/stock/history/quote';
    const queryParams = {
        symbol: 'AAPL',
        date: '20240105',
        interval: '1h',
    ...
    try {
        ...
        // Save to fixtures
        const fixtureDir = path.join(__dirname, '../../test/fixtures/stock-history-quote');
